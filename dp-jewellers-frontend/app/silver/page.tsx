...
export default function SilverPage() {
  ...
  useEffect(() => {
    fetch("/api/backend/products").then(r=>r.json()).then(data=>{
      const silver = data.filter((p:any)=> p.category?.toUpperCase() === "SILVER");
      console.log("SILVER PRODUCTS:", silver);
      setProducts(silver);
    });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-black text-white"><Navbar />
      <div className="px-6 md:px-20 py-6">
        ...
        <h1 className="text-center text-3xl text-[#f7d774]">SILVER - {list.length} Products</h1>
